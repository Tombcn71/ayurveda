...
export default async function ProductPage({
  ...
  // Wacht op de parameters (nodig in Next.js 16)
  const { id } = await params;

  // Beveiliging: stop als er geen ID is tijdens het bouwen
  if (!id) return null;

  let product;
  try {
    // Haal het product op uit WooCommerce
    ...
    return (
      <div className="p-20 text-center text-red-500 font-serif">
        ...
        <p>
          No se pudo cargar el producto. Por favor, inténtalo de nuevo más
          tarde.
        </p>
      </div>
    );
  ...
  // Als de API leeg terugkomt
  if (!product) {
    return (
      <div className="p-20 text-center font-serif text-gray-500">
